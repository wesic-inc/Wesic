...
class Form
{
    ...
    public static function renderBasic($form, $data)
    {
        ...
        foreach ($form["struct"] as $name => $option) {
            if ($option["type"] == "text"|| $option["type"]=="password" || $option["type"]=="email") {
                $output .= self::input($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"]=="textarea") {
                $output .= self::text($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"]=="texteditor") {
                $output .= self::texteditor($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"]=="select") {
                $output .= self::select($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "datetime-local" || $option["type"] == "date" || $option["type"] == "time") {
                $output .= self::date($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "submit") {
                $output .= self::submit($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "link") {
                $output .= self::link($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "radio") {
                $output .= self::radio($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "info") {
                $output .= self::info($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "title") {
                $output .= self::title($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "custom-datepicker") {
                $output .= self::datepicker($name, $option, isset($data[$name])?$data[$name]:"");
            } elseif ($option["type"] == "separator") {
                $output .= '<div class="separator"></div>';
            } elseif ($option["type"] == "captcha") {
                $output .= self::captcha($name, $option, $isset($data[$name])?$data[$name]:"");
            }
        }
        ...
        foreach ($form["groups"] as $group => $children) {
            ...
            foreach ($children as $child) {
                ...
                if ($form["struct"][$child]["type"] == "text"|| $form["struct"][$child]["type"] =="password") {
                    $step .= self::input($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] =="textarea") {
                    $step .= self::text($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] =="select") {
                    $step .= self::select($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                    ...
                    $step .= self::date($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] == "submit") {
                    $step .= self::submit($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] == "separator") {
                    $step .= '<div class="separator"></div>';
                } elseif ($form["struct"][$child]["type"] == "link") {
                    $step .= self::link($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] == "info") {
                    $step .= self::info($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] == "title") {
                    $step .= self::title($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                    ...
                    $step .= self::captcha($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                } elseif ($form["struct"][$child]["type"] == "radio") {
                    $step .= self::radio($child, $form["struct"][$child], isset($data[$child])?$data[$child]:"" );
                }
                $step .= "</li>";
            }
    ...
    public static function datepicker($name, $option, $data)
    {
		...
		<option value="02" '.(($data['mm']=="02")||date("m")=="02"?'selected="selected"':"").'>02-FÃ©v</option>
		...
		<option value="08"" '.(($data['mm']=="08")||date("m")=="08"?'selected="selected"':"").'>08-AoÃ»t</option>
		...
		<option value="12" '.(($data['mm']=="12")||date("m")=="12"?'selected="selected"':"").'>12-DÃ©c</option>
		</select>
